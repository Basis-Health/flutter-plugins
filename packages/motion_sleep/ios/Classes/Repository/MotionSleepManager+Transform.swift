//
//  MotionSleepManager+Transform.swift
//  motion_sleep
//
...
extension Array where Element == CMMotionActivity {
    func toSleepSessions() -> [SleepSession] {
        ...
        for activity in self {
            if let lastResult = results.last {
                if activity.startDate > lastResult.end && activity.isEligibleForSleep() {
                    if let nextActivity = self.first(where: {
                        $0.startDate > activity.startDate && $0.isEligibleForSleepInterval(lowerBound: activity)
                    }) {
        ...
        return results.compactMap({
            SleepSession(type: .asleep, startDate: $0.start, endDate: $0.end, source: SleepSessionSource.motion)
        })
    }
    ...
    func toEligibleSleepSessions(sleepTime: SleepTime) -> [SleepSession] {
        ...
        let filteredSleepSessions = sleepSessions.filter { sleepSession -> Bool in
            ...
            guard let sleepStartTime = self.date(
                date: sleepSession.interval.start,
                with: sleepTime.startingHours,
                minute: sleepTime.startingMinutes,
                calendar: calendar
            ),
                  let sleepEndTime = self.date(
                    date: sleepSession.interval.end,
                    with: sleepTime.endingHours,
                    minute: sleepTime.endingMinutes,
                    calendar: calendar
                  ),
            ...
            return sleepTimeInterval.contains(sleepSession.startDate)
            && sleepTimeInterval.contains(sleepSession.endDate)
        }
    ...
    func aggregate(
        ...
        return results.compactMap({
            SleepSession(type: .asleep, startDate: $0.start, endDate: $0.end, source: .motion)
        })
    }
}
...
extension CMMotionActivity {
    func isEligibleForSleep() -> Bool {
        ...
        if self.stationary && self.confidence == .high {
            return true
        }
        return false
    }
    ...
    func isEligibleForSleepInterval(lowerBound: CMMotionActivity) -> Bool {
